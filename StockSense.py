# ⭐StockSense Web App by SREENISH, SHREYAS, ATIQ.
...
st.write('''# ⭐StockSense ''')  # title
st.sidebar.image("stock_icon.jpeg", width=250,
                 use_column_width=False)  # logo
st.sidebar.write('''# ⭐StockSense ''')
...
# read csv file
stock_df = pd.read_csv("StockSenseTickersData.csv")
...
if(selected == 'Stocks Performance Comparison'):  # if user selects 'Stocks Performance Comparison'
    ...
    dict_csv = pd.read_csv('StockSenseTickersData.csv', header=None, index_col=0).to_dict()[1]  # read csv file
...
# Stock Price Prediction Section Ends Here
    ...
    st.markdown('<p class="big-font">⭐StockSense is a web application that allows users to visualize Stock Performance Comparison, Real-Time Stock Prices and Stock Price Prediction. This application is developed using Streamlit. Streamlit is an open source app framework in Python language. It helps users to create web apps for Data Science and Machine Learning in a short time. This Project is developed by Vaishnavi Sharma and Rohit More. You can find more about the developers on their GitHub Profiles shared below.<br>Hope you are able to employ this application well and get your desired output.<br> Cheers!</p>', unsafe_allow_html=True)
    st.subheader('Sreenish Sharma [![Repo](https://badgen.net/badge/icon/GitHub?icon=github&label)](https://github.com/SreenishSharma83) ')
    st.subheader('Atiq Shaik [![Repo](https://badgen.net/badge/icon/GitHub?icon=github&label)](https://github.com/atiq2409) ')
    st.subheader('Shreyas Mittapalli [![Repo](https://badgen.net/badge/icon/GitHub?icon=github&label)](https://github.com/ShreyasMittapalli) ')
